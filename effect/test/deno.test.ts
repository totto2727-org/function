import { afterAll, describe, expect, it, layer } from "./deno.ts";
import {
  Context,
  Duration,
  Effect,
...
it.effect.runIf(true)(
  "effect runIf (true)",
  () => Effect.sync(() => expect(1).toEqual(1)),
);

it.effect.runIf(false)(
  "effect runIf (false)",
...
describe("layer", () => {
  ...
  layer(Sleeper.Default)("test services", (it) => {
    it.effect("TestClock", () =>
      Effect.gen(function* () {
        const fiber = yield* Effect.sleep("1 minutes").pipe(
          Effect.timeoutTo({
            duration: "3 minutes",
            onSuccess: () => Option.some("success"),
            onTimeout: () => Option.none<void>(),
          }),
          Effect.fork,
        );
        yield* TestClock.adjust("2 minutes");
        const option = yield* Fiber.join(fiber);
        const actual = yield* option;
        expect(actual).toEqual("success");
        console.log("TestClock done");
      }));
  });
...
it.effect.prop("symmetry with object", {
  a: realNumber,
  b: FastCheck.integer(),
}, ({ a, b }) =>
  Effect.gen(function* () {
